...
# ProfileLoader 实例（load_research_config 调用后设置，含 priority 乘数支持）
_PROFILE_LOADER = None


def load_research_config(config_path: str) -> Dict:
    """
    从 YAML 文件加载研究兴趣配置，同时构建 ProfileLoader 实例。

    Args:
        config_path: 配置文件路径

    Returns:
        研究配置字典（同时设置模块级 _PROFILE_LOADER 以供评分函数使用）
    """
    import yaml

    global S2_API_KEY, _PROFILE_LOADER

    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            config = yaml.safe_load(f)

        # 读取 Semantic Scholar API Key（支持 v1 顶层 和 v2 system.* 两种位置）
        S2_API_KEY = (
            config.get('semantic_scholar_api_key')
            or config.get('system', {}).get('semantic_scholar_api_key')
        )

        # 构建 ProfileLoader（含 priority 乘数支持）
        try:
            # 确保 profile_loader 在同目录可导入
            _scripts_dir = os.path.dirname(os.path.abspath(__file__))
            if _scripts_dir not in sys.path:
                sys.path.insert(0, _scripts_dir)
            from profile_loader import ProfileLoader
            _PROFILE_LOADER = ProfileLoader(config_path)
            logger.info("ProfileLoader v2 loaded: %d domains", len(_PROFILE_LOADER.domains))
        except Exception as e:
            logger.warning("ProfileLoader not available, falling back to legacy scoring: %s", e)
            _PROFILE_LOADER = None
...
def search_semantic_scholar_hot_papers(
    ...
    end_date: datetime,
    top_k: int = 20,
    max_retries: int = 2,   # V1.1: 默认重试次数从3降为2，配合外层熔断器
    api_key: str | None = None,  # V1.1: 支持外部传入 API Key
) -> List[Dict]:
    """
    ...
    # V1.1: 支持外部传入 api_key（优先级高于全局 S2_API_KEY）
    effective_key = api_key or S2_API_KEY
    if effective_key:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                # 对于 429 错误，使用更短的等待时间（V1.1 优化：从 30s 降为 10s）
                if is_rate_limit:
                    wait_time = min(S2_RATE_LIMIT_WAIT, 10)
                ...
                # V1.1: 最终失败时检查熔断器状态
                try:
                    from cache_manager import S2CircuitBreaker
                    if S2CircuitBreaker.is_open():
                        logger.warning("[S2] 熔断器已开路，后续查询将跳过")
                except ImportError:
                    try:
                        from scripts.cache_manager import S2CircuitBreaker
                        if S2CircuitBreaker.is_open():
                            logger.warning("[S2] 熔断器已开路，后续查询将跳过")
                    except ImportError:
                        pass
...
def search_hot_papers_from_categories(
    ...
    V1.1 改造：
      - 每次 query 前检查 S2CircuitBreaker.is_open()，开路则立即中断循环
      - 委托给 cache_manager.fetch_s2_papers_with_cache()（含缓存 + 熔断记录）
      - 移除固定 sleep(3)，由缓存命中大幅减少实际 API 调用次数

    Args:
        categories:          arXiv 分类列表
        start_date:          开始日期
        end_date:            结束日期
        top_k_per_category:  每个查询返回的论文数
    ...
    # 延迟导入，避免循环依赖
    try:
        from cache_manager import S2CircuitBreaker, fetch_s2_papers_with_cache
        _use_cache = True
    except ImportError:
        try:
            from scripts.cache_manager import S2CircuitBreaker, fetch_s2_papers_with_cache
            _use_cache = True
        except ImportError:
            _use_cache = False
            logger.warning("cache_manager 不可用，回退到旧版 S2 调用逻辑")
    ...
    for query in unique_queries:
        # ── V1.1: 每次循环前检查熔断器 ──────────────────────────────────
        if _use_cache and S2CircuitBreaker.is_open():
            logger.warning("[S2] 熔断器开路，终止剩余 %d 个 query", len(unique_queries))
            break

        if _use_cache:
            papers = fetch_s2_papers_with_cache(
                query=query,
            ...
            if papers:  # 仅旧版路径需要限速 sleep
                time.sleep(S2_CATEGORY_REQUEST_INTERVAL)

        # 去重（基于 arXiv ID）
        for p in papers:
            arxiv_id = p.get("arxiv_id") or (
                p.get("externalIds", {}).get("ArXiv") if p.get("externalIds") else None
            )
            p["arxiv_id"] = arxiv_id
...
def calculate_relevance_score(
    ...
    _profile_loader=None,  # 优先使用 ProfileLoader（含 priority 乘数）
) -> Tuple[float, Optional[str], List[str]]:
    """
    计算论文与研究兴趣的相关性评分。

    **修复说明**：原实现忽略了 domain.priority 字段导致高/低优先级领域评分相同。
    现在若传入 _profile_loader（ProfileLoader 实例），则使用其 score_relevance()
    方法进行带 priority 乘数的加权评分；否则退回旧逻辑（向后兼容）。

    Args:
        paper:           论文信息
        domains:         研究领域配置字典（v1 兼容用）
        excluded_keywords: 排除关键词
        _profile_loader: ProfileLoader 实例（推荐传入）
    ...
    # --- 优先路径：使用 ProfileLoader（带 priority 乘数）---
    if _profile_loader is not None:
        result = _profile_loader.score_relevance(title, summary, categories)
        return result.weighted_score, result.domain, result.matched_keywords

    # --- 回退路径：旧逻辑（向后兼容，不含 priority）---
...
def filter_and_score_papers(
    ...
    若 _PROFILE_LOADER 已初始化（即 load_research_config 成功加载了 profile.yaml v2），
    则使用带 priority 乘数的加权评分；否则回退到旧版逻辑。
    ...
    for paper in papers:
        # 计算相关性（传入 _PROFILE_LOADER 以启用 priority 乘数）
        relevance, matched_domain, matched_keywords = calculate_relevance_score(
            paper, domains, excluded_keywords, _profile_loader=_PROFILE_LOADER
        )
        ...
        # 标记是否触发深度分析（来自 ProfileLoader）
        if _PROFILE_LOADER is not None:
            paper['trigger_deep_analysis'] = _PROFILE_LOADER.should_trigger_deep_analysis(
                recommendation_score, matched_domain
            )
        else:
            paper['trigger_deep_analysis'] = False
